...
<Remark type="note" display="compact">
  The below GIF is using the `"Both"` option.
</Remark>
...
Whether [addons](/wiki/addons) can be automatically detected and applied. Addons can set up the environment to be as close as possible to your target runtime environment.
...
An array of absolute/workspace-relative paths that will be added to the workspace diagnosis - meaning you will get completion and context from these files. Can be a file or directory. Files included here will have some features disabled such as renaming fields to prevent accidentally renaming your library files.
