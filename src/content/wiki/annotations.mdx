...
Generics allow code to be reused and serve as a sort of "placeholder" for a type. Surrounding the generic in backticks (<code>`</code>) will capture the string value from the caller and use it for the type. [Generics are still WIP](https://github.com/LuaLS/lua-language-server/issues/1861).
...
---@return T # The return type will be the type identified by the string value of the 'class' argument
local function newWithCapture(class) end

-- obj1 has type 'Vehicle' here, because "Vehicle" is captured from the first argument
...
-- obj2 has type 'string' here, because the first argument is a string
...
<Remark type="note">
  ...
  recommended to instead write multiple function definitions, one for each
  needed signature with its [`@param`](#param) and [`@return`](#return)
  annotations. This allows the functions to be as detailed as possible. Because
  the functions do not exist at runtime, this is acceptable.
</Remark>
